...
class PU_RO(Bot):
    ...
    If the bot is the follower, the bot plays a random card.
    """    
    def __init__(
        self, rand: random.Random, name: Optional[str] = "PU_RO"
    ) -> None:
        super().__init__(name)
        self.rng = rand
...
class PU_ARO(Bot):
    """
    This is a two-stage bot. In the first stage, it applies PU_RO bot. In the second stage, it uses Alphbeta Bot.
    ...
    Returns:
        A Move object
    """ 
    def __init__(self, rand: random.Random, name: Optional[str] = "PU_ARO") -> None:
        super().__init__(name)
        self.rng = rand
        self.bot_phase1: Bot = PU_RO(rand=self.rng, name="PU_RO")
