...
class PU_R(Bot):
    """
    This bot is designed for Phase 1 of the game, where one strategy is applied.
    In cases where the bot is the leader:
    The risk factor strategy (referred to as R), which adjusts the value of the utility heuristic based on the game state after calculating 
    the probability of the opponent not having dangerous cards.
    If the bot is the follower,it plays a random move.
    """
    def __init__(
        self, rand: random.Random, name: Optional[str] = "P_R"
    ) -> None:
        super().__init__(name)
        self.rng = rand
...
class PU_AR(Bot):
    """
    This is a two-stage bot. In the first stage, it applies PU_R bot. In the second stage, it uses Alphbeta Bot.
    ...
    """
    def __init__(
        self, rand: random.Random, name: Optional[str] = "PU_AR"
    ) -> None:
        super().__init__(name)
        self.rng = rand
        self.bot_phase1: Bot = PU_R(
            rand=self.rng, name="PU_R"
        )
