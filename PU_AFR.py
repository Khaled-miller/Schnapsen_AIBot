...
class PU_FR(Bot):
    ...
    The bot uses the risk factor strategy (referred to as R), which adjusts the value of the utility heuristic based on the game state.
    ...
    """
    def __init__(
        self, rand: random.Random, name: Optional[str] = "PU_FR"
    ) -> None:
        super().__init__(name)
        self.rng = rand
...
class PU_AFR(Bot):
    """
    This is a two-stage bot. In the first stage, it applies PU_FR bot. In the second stage, it uses Alphbeta Bot.
    ...
    def __init__(
        self, rand: random.Random, name: Optional[str] = "PU_AFR"
    ) -> None:
        super().__init__(name)
        self.rng = rand
        self.bot_phase1: Bot = PU_FR(
            rand=self.rng, name="PU_FR"
        )
